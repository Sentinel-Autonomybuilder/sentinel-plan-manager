...
export const KEPLR_COOKIE_NAME = 'spm_keplr';
...
// ─── Keplr Session ───────────────────────────────────────────────────────────
// Keplr holds the private key in the browser extension. The server never
// touches the mnemonic; signing happens client-side and the server only
// broadcasts the pre-signed bytes via the existing RPC failover. The session
// shape mirrors mnemonic sessions so safeBroadcast can branch on `kind`.

const keplrCache = new Map();

/**
 * Build a Keplr-backed session keyed by bech32 address. The session has no
 * wallet/getClient — safeBroadcast detects `kind === 'keplr'` and returns a
 * signDoc payload for the browser to sign instead of calling signAndBroadcast.
 *
 * @param {string} addr - sent1... bech32 address from window.keplr.getKey
 * @param {string} pubkeyB64 - base64 secp256k1 compressed pubkey
 */
export function keplrSessionFromAddress(addr, pubkeyB64) {
  if (!addr || !addr.startsWith('sent1')) throw new Error('Invalid Keplr address');
  let session = keplrCache.get(addr);
  if (session) {
    keplrCache.delete(addr); keplrCache.set(addr, session);
    if (pubkeyB64 && pubkeyB64 !== session.pubkeyB64) session.pubkeyB64 = pubkeyB64;
    return session;
  }
  ...
  session = {
    ...
    addr,
    provAddr,
    pubkeyB64: pubkeyB64 || null,
    wallet: null,
    broadcastQueue: Promise.resolve(),
    async getClient() {
      throw new Error('client-signs');
    },
    async resetClient() {
      throw new Error('client-signs');
    },
  ...
  while (keplrCache.size >= CACHE_MAX) {
    const oldest = keplrCache.keys().next().value;
    keplrCache.delete(oldest);
  }
  keplrCache.set(addr, session);
  return session;
}

export function dropKeplrSessionFromCache(addr) {
  try { keplrCache.delete(addr); } catch {}
}
...
/**
 * Keplr cookie carries `addr|pubkeyB64`, signed (HMAC-SHA256) with the same
 * session key so a tampered cookie can't promote an arbitrary address into a
 * signing session. No mnemonic, no encryption needed — the value is public.
 */
...
export function parseKeplrToken(token) {
  if (!_key) throw new Error('Session key not initialized');
  ...
  return { addr, pubkeyB64: pubkeyB64 || null };
}

export function buildSetKeplrCookie(token, { secure }) {
  const attrs = [
    `${KEPLR_COOKIE_NAME}=${token}`,
    'HttpOnly',
    'SameSite=Strict',
    'Path=/',
    `Max-Age=${COOKIE_MAX_AGE_SEC}`,
...
export function buildClearKeplrCookie({ secure }) {
  const attrs = [
    `${KEPLR_COOKIE_NAME}=`,
    'HttpOnly',
    'SameSite=Strict',
